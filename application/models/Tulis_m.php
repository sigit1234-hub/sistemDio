...
class Tulis_m extends CI_Model
{
    ...
    {
        return $this->db->order_by('id_dokumen', 'DESC')->get('dokumen')->result_array();
    }
    ...
    public function dataPengajuan($table, $limit, $start)
    {
        ...
        return $this->db->order_by('id_dokumen', 'DESC')->get($table, $limit, $start)->result_array();
    }
    ...
    {
        $this->db->select('*');
        $this->db->from('approval_checker');
        $this->db->join('approval_signer', 'approval_signer.id_dok = approval_checker.id_dok');
